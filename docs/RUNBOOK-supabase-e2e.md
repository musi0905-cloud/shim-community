...
> **왜 사람이 직접 실행하는가**
> 개발 세션이 도는 컨테이너는 조직 egress 정책상 `supabase.co` / `supabase.com`
> 로 나갈 수 없다(프록시가 CONNECT 에 403 응답). 컨테이너 이미지 레지스트리의
> blob CDN 도 막혀 있어 로컬 Supabase 스택도 띄울 수 없다. 그래서 실제 연결이
> 필요한 단계는 네트워크가 열린 곳에서 실행해야 한다.
